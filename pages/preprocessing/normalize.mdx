...
<p className='formula'>$y_i^{norm} = \frac{y_i - \min(y)}{\max(y) - \min(y)} \times (M - m) + m$</p>
...
- **Peak**: The user-defined intensity value to which
  the spectrum will be normalized.

<p className='formula'>$y_i^{norm} = \frac{y_i}{y_{peak}}$</p>
